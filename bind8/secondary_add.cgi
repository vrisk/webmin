...
foreach my $z (@zoneconfs) {
	...
	if ($type eq "primary" || $type eq "primary") {
		$zmap{$z->{'value'}} = $z;
		}
	}

# Make sure each host is set up for BIND
my @zones = grep { $_->{'type'} eq 'primary' } &list_zone_names();
foreach my $s (@add) {
	...
	&add_secondary_server($s);
	my %rgot = map { $_->{'name'}, 1 } @rzones;

	if ($in{'sync'}) {
		# Add all primary zones from this server to the secondary
		my $zcount = 0;
		my $zerr = 0;
		my $sip = &to_ipaddress($s->{'host'});
		my %zerrs;
		foreach my $zone (grep { !$rgot{$_->{'name'}} } @zones) {
			my ($secondaryerr) = &create_on_secondarys($zone->{'name'},
			  $myip, undef, [ $s->{'host'} ], $zone->{'view'});
			if ($secondaryerr) {
				$zerrs{$secondaryerr->[0]->{'host'}} ||=
					$secondaryerr->[1];
				$zerr++;
				}
			else {
		...
		# Add secondary IP to primary zone allow-transfer and also-notify
		# blocks
		my $dchanged = 0;
...
&ui_print_footer("list_secondarys.cgi", $text{'secondarys_return'});
