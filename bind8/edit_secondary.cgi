#!/usr/local/bin/perl
# edit_secondary.cgi
# Display records and other info for an existing secondary or stub zone
use strict;
use warnings;
...
if ($zone->{'file'}) {
	...
	$lasttrans = &text('secondary_last', @st && $st[7] ? &make_date($st[9])
					     	       : $text{'secondary_never'});
	}
&ui_print_header(&zone_subhead($zone),
		 $0 =~ /edit_secondary/ ? $text{'secondary_title'} : $text{'stub_title'},
...
if ($access{'file'} && $zone->{'file'}) {
	...
	push(@titles, $text{'secondary_manual'});
...
push(@titles, $text{'secondary_xfer'});
...
if (!$access{'ro'} && ($access{'delete'} || $apply)) {
	...
	if ($access{'primary'} && $st[7]) {
		print &ui_buttons_row("convert_secondary.cgi",
			$text{'secondary_convert'},
			$text{'secondary_convertdesc'},
	...
	if ($access{'delete'}) {
		print &ui_buttons_row("delete_zone.cgi",
			$text{'primary_del'}, $text{'secondary_delmsg'},
			&ui_hidden("zone", $in{'zone'}).
			&ui_hidden("view", $in{'view'}));
		}
