#!/usr/local/bin/perl
# edit_primary.cgi
# Display options and directives in an existing primary zone
use strict;
use warnings;
...
&ui_print_header(&zone_subhead($zone), $text{'primary_title'}, "",
		 undef, undef, undef, undef, &restart_links($zone));

my $d = &get_virtualmin_domains($dom);
if ($d && $d->{'alias'}) {
	print &ui_alert_box($text{'primary_vminalias'}, 'danger');
	}
elsif ($d) {
	print &ui_alert_box($text{'primary_vmin'}, 'warn');
	}
...
if ($access{'whois'} && &has_command($config{'whois_cmd'}) &&
	...
	push(@titles, $text{'primary_whois'});
...
if (!$access{'ro'} && ($access{'delete'} || $apply)) {
	...
	if ($apply) {
		...
		print &ui_buttons_row(
			"freeze_zone.cgi", $text{'primary_freeze'},
			$text{'primary_freezemsg2'},
		...
		print &ui_buttons_row(
			"unfreeze_zone.cgi", $text{'primary_unfreeze'},
			$text{'primary_unfreezemsg2'},
			&ui_hidden("zone", $in{'zone'}).
			&ui_hidden("view", $in{'view'})
			);
		}
	...
	if (&supports_check_zone()) {
		print &ui_buttons_row(
			"check_zone.cgi", $text{'primary_checkzone'},
			$text{'primary_checkzonemsg'},
			&ui_hidden("zone", $in{'zone'}).
			&ui_hidden("view", $in{'view'})
			);
		}
	...
	# Convert to secondary zone
	if ($access{'secondary'}) {
		print &ui_buttons_row("convert_primary.cgi",
			$text{'primary_convert'},
			$text{'primary_convertdesc'},
			&ui_hidden("zone", $in{'zone'}).
			&ui_hidden("view", $in{'view'}));
		}
	...
	if ($access{'delete'}) {
		...
		print &ui_buttons_row(
			"delete_zone.cgi", $text{'primary_del'},
			$text{'primary_delmsg'}." ".
			($dom !~ /in-addr\.arpa/i &&
			 $dom !~ /\.$ipv6revzone/i ? $text{'primary_delrev'}
						   : ""),
			&ui_hidden("zone", $in{'zone'}).
			&ui_hidden("view", $in{'view'})
			);
